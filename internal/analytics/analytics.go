...
type Summary struct {
	...
	ProbabilityRandomChance   *float64 `json:"probability_random_chance"`
...
func Calculate(trades []positions.RoundTrip, tolerance float64, minimumKelly int) Summary {
	...
	if decisive > 0 {
		...
		chance := binomialTwoSided(s.Wins, decisive)
		s.ProbabilityRandomChance = &chance
	}
...
func binomialTwoSided(wins, total int) float64 {
	if total == 0 {
		return 1
	}
	...
	for i := 0; i <= k; i++ {
		...
		logRest, _ := math.Lgamma(float64(total - i + 1))
		tail += math.Exp(logN - logI - logRest - float64(total)*math.Ln2)
	}
