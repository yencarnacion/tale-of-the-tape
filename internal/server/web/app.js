...
async function dashboard() {
  ...
  const detailed = [
    ...
    ['Trade P&L Standard Deviation', summary.trade_pnl_standard_deviation == null ? 'N/A' : money(summary.trade_pnl_standard_deviation * 1e6)], ['System Quality Number (SQN)', decimal(summary.system_quality_number)], ['Probability True Daily Expectancy Is Positive', percent(summary.probability_positive_expectancy)],
    ['Daily Expectancy 90% Credible Interval', summary.expectancy_daily_lower_90 == null ? 'N/A' : `${money(summary.expectancy_daily_lower_90 * 1e6)} to ${money(summary.expectancy_daily_upper_90 * 1e6)}`],
...
async function calendar() {
  ...
  for (let date = new Date(gridStart); date <= gridEnd; date.setDate(date.getDate() + 1)) {
    ...
    const showBoundaryDate = !inMonth && included && (dateStart || dateEnd);
    html += inMonth || showBoundaryDate ? `<div class="day ${!inMonth ? 'outside-month ' : ''}${included ? `clickable ${net > 10000 ? 'positive' : net < -10000 ? 'negative' : 'scratch'}` : 'filtered-out'}"${included ? ` onclick="showDay('${key}')"` : ''}><b>${date.getDate()}</b>${item ? `<br>${money(net)}<br><small>${item.trades} trades</small>` : ''}</div>` : '<div></div>';
    if (date.getDay() === 6) {
